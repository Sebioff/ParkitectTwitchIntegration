...
namespace IrcDotNet
{
    ...
    public abstract partial class IrcClient : IDisposable
    {
        ...
        {
			SendMessageKick(channelUsers.Select(cu => IrcDotNetTuple.Create(cu.Channel.Name, cu.User.NickName)), comment);
        }
        ...
        internal void Join(IEnumerable<IrcDotNetTuple<string, string>> channels)
        {
            SendMessageJoin(channels);
        }
        ...
        protected IrcDotNetTuple<string, string> GetUserModeAndNickName(string input)
        {
            if (input == null)
            ...
            if (this.channelUserModesPrefixes.TryGetValue(input[0], out mode))
				return IrcDotNetTuple.Create(input.Substring(1), mode.ToString());
            else
				return IrcDotNetTuple.Create(input, string.Empty);
        }
        ...
        protected IrcDotNetTuple<string, IEnumerable<string>> GetModeAndParameters(IEnumerable<string> messageParameters)
        {
			...
			return IrcDotNetTuple.Create(modes.ToString(), (IEnumerable<string>)modeParameters.AsReadOnly());
        }

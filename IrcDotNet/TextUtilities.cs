...
namespace IrcDotNet
{
    ...
    internal static class TextUtilities
    {
        ...
        public static IrcDotNetTuple<string, string> SplitIntoPair(this string value, string separator)
        {
            var index = value.IndexOf(separator);
            if (index < 0)
				return IrcDotNetTuple.Create(value, (string)null);
            else
				return IrcDotNetTuple.Create(value.Substring(0, index), value.Substring(index + separator.Length));
        }
